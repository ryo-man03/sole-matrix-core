  ...
  return (
    <section className="core-v1-panel" aria-labelledby="core-v1-recommendation-title">
      ...
      {result ? <div className="core-v1-result">
        ...
        <section className="core-v1-explanation" aria-labelledby="core-v1-explanation-title"><p className="diagnosis-summary-kicker">Explanation</p><h4 id="core-v1-explanation-title">判断の理由</h4><p>{result.explanation.summary}</p><p className="core-v1-provider-note">{result.explanation.source === "gemini" ? "Geminiの補助説明をshape validation後に表示しています。最終DecisionはCoreが決定します。" : "Geminiを安全に利用できなかったため、rule-based説明を表示しています。"}</p><div className="core-v1-explanation-columns"><ExplanationList title="理由" items={result.explanation.reasons} /><ExplanationList title="注意点" items={result.explanation.cautions} /></div></section>
        ...
        <section className="core-v1-readiness" aria-labelledby="core-v1-readiness-title"><p className="diagnosis-summary-kicker">Readiness</p><h4 id="core-v1-readiness-title">外部APIの状態</h4><div><strong data-status={result.readiness.gemini.status}>Gemini: {result.readiness.gemini.status}</strong><p>{result.readiness.gemini.detail}</p></div><div><strong data-status={result.readiness.rakuten.status}>Rakuten: {result.readiness.rakuten.status}</strong><p>{result.readiness.rakuten.detail}</p></div></section>
