...
export async function PUT(request: Request) {
  ...
  try { const input = parseConsentUpdate(await readBoundedJsonBody(request)); await recordConsent(user.id, input.type, input.granted); return NextResponse.json({ ok: true, data: input }); }
