...
describe("fit-confidence route", () => {
  ...
  it("reads only the authenticated user's owned sneakers and sizes", async () => {
    vi.mocked(listOwnedRows)
      .mockResolvedValueOnce([{ brand: "New Balance", model_name: "991v2", style_code: "U991GL2", size_system: "JP", size_value: 26 }])
    ...
    expect(await response.json()).toMatchObject({ ok: true, data: { fit: { state: "strong_reference" } } });
  });
